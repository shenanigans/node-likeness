...
var TYPE_TRANSFORMERS = {
    object:     function (target, value, path, root, errMessage) {
        if (typeof value == 'string') {
            (function(){
                try {
                    value = JSON.parse (value);
                } catch (err) {
                    throw errMessage || 'failed to cast String to Object';
                }
            })();
        ...
        for (var nameI=0,j=names.length; nameI<j; nameI++) {
            ...
            if (childSchema) {
                ...
                if (childSchema.constraints.fill || childSchema.constraints.list)
                    ...
                    nextValue = childSchema.transform (nextValue, newValue, path, root, errMessage);
    ...
    array:      function (target, value, path, root, errMessage) {
        path = path || this.path;
        if (typeof value == 'string') {
            (function(){
                try {
                    ...
                    throw errMessage || 'failed to cast String to Array';
                }
            })();
    ...
    string:     function (target, value, path, root, errMessage) {
        path = path || this.path;
        if (typeof value != 'string')
    ...
    number:     function (target, value, path, root, errMessage) {
        path = path || this.path;
        if (typeof value == 'string') {
            // cast
            if (!value.match (RE_VALID_NUMBER))
                throw errMessage || 'could not convert String to Number';
    ...
    boolean:    function (target, value, path, root, errMessage) {
        path = path || this.path;
        if (typeof value == 'string') {
            // cast
            ...
            if (canonical == 'true')
                ...
                value = false;
            else throw errMessage || 'could not convert String to Boolean';
        }
    ...
    null:       function (target, value, path, root, errMessage) {
        return value;
    }
};
