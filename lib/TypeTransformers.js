    ...
    array:      function (value, target, mongoloid, callback, path) {
        ...
        if (callback && ( this.constraints.all || this.constraints.sequence )) {
            ...
            return async.times (value.length, function (subvalueI, callback) {
                var subvalue = value[subvalueI];
                if (self.constraints.all)
                    return self.constraints.all.transform (
                        subvalue,
                        target[subvalueI],
                        undefined,
                        function (err, newSubvalue) {
                            if (err)
                                return callback (new TransformError (
                                    'INVALID',
                                    value,
                            ...
                            target[subvalueI] = newSubvalue;
                            if (!self.constraints.sequence) return callback();
                            if (self.constraints.sequence[subvalueI])
                                return self.constraints.sequence[subvalueI].transform (
                                    subvalue,
                                    target[subvalueI],
                                    undefined,
                                    function (err, newSubvalue) {
                            ...
                            if (self.constraints.extras)
                                return self.constraints.extras.transform (
                                    subvalue,
                                    target[subvalueI],
                                    undefined,
                                    function (err, newSubvalue) {
                ...
                // .sequence and .extras without .all
                if (self.constraints.sequence[subvalueI])
                    return self.constraints.sequence[subvalueI].transform (
                        subvalue,
                        target[subvalueI],
        ...
        // sync
        // .all
        if (this.constraints.all || this.constraints.sequence || this.constraints.exists) {
            var newValue = [];
            for (var i=0,j=value.length; i<j; i++) {
        ...
        if (this.constraints.exists) {
            ...
            for (var i=0,j=this.constraints.exists.length; i<j; i++) {
                var constraint = this.constraints.exists[i];
                exists.push ([ constraint.constraints.times || 1, constraint ]);
            }
            for (var i=0,j=target.length; i<j; i++) {
                ...
                for (var k=0,l=exists.length; k<l; k++)
                    try {
                        console.log ('exists', value[i], target[i]);
                        target[i] = exists[k][1].transform (
                            value[i],
                            target[i],
                            undefined,
                            path
                        );
                        console.log ('accepted', target[i], exists[k][0]);
                        if (!--exists[k][0]) {
                            exists.splice (k, 1);
                            k--; l--;
                            console.log ('finished ('+exists.length+' remaining)');
                        }
                    } catch (err) {  }
            }
            if (exists.length)
