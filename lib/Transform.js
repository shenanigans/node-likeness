...
@returns
    ...
    value.
*/
function transform (/* target, value, path, root, errContext */) {
    ...
    if (this.constraints.recurse)
        return this.constraints.recurse.transform (target, value, path, root, errMessage);
    ...
    var newVal = TYPE_TRANSFORMERS[canonType]
        .call (this, target, value, path, root, errMessage)
        ;
        ...
        for (var i=0, j=anyOf.length; i<j; i++)
            if ( (function(){
                try {
                    ...
                    newVal = anyOf[i].transform (target, newVal, path, root, errMessage);
                    ok = true;
                    return true;
    ...
    if (this.constraints.oneOf) {
        ...
        for (var i=0, j=oneOf.length; i<j; i++)
            (function(){
                try {
                    newVal = oneOf[i].transform (target, newVal, path, root, errMessage);
                    if (found)
                        throw BOGUS;
                    found = true;
                } catch (err) {
    ...
    if (this.constraints.not) {
        ...
        (function(){
            try {
                not.transform (target, newVal, path, root, errMessage);
                doThrow = true;
