...
var TYPE_VALIDATORS = {
            ...
            return async.each (keys, function (name, callback) {
                ...
                function postValidate (err) {
                    ...
                    if (self.constraints.dependencies) {
                        var depKeys = Object.keys (self.constraints.dependencies);
                        if (depKeys.length && self.constraints.dependencies[depKeys[0]] instanceof Array) {
                            // keys depend on arrays of keys
                            for (var i=0,j=depKeys.length; i<j; i++) {
                                if (!Object.hasOwnProperty.call (value, depKeys[i]))
                                    continue;
                                var required = self.constraints.dependencies[depKeys[i]];
                                for (var k=0,l=required.length; k<l; k++)
                                    if (!Object.hasOwnProperty.call (value, required[k]))
                                        throw new ValidationError (
                                            ...
                                            value,
                                            self.path,
                                            'key depencies not met for key ' + depKeys[i]
                                        );
                            }
                        } else
                            jobs.push (function (callback) {
                                ...
                                for (var i=0,j=depKeys.length; i<j; i++)
                                    if (Object.hasOwnProperty.call (value, depKeys[i]))
                                        active.push (self.constraints.dependencies[depKeys[i]]);
                                async.each (active, function (dependentSchema, callback) {
                                    dependentSchema.validate (value, callback);
                                }, callback);
                            });
                    }

                    // .exists
                    if (exists && exists.length) // process .exists constraints
                        jobs.push (function (callback) {
                            async.times (exists.length, function (existsI, callback) {
                                if (!existsLives[existsI])
                                    return callback();

                                exists[existsI].validate (childValue, function (err) {
                    ...
                    if (self.constraints.all)
                        jobs.push (function (callback) {
                            self.constraints.all.validate (childValue, function (err) {
                                if (err) return callback (err);
                                callback();
                            });
                        });
                ...
                if (Object.hasOwnProperty.call (self.children, name))
                    return self.children[name].validate (childValue, postValidate);
                else {
                    // unknown key
                    ...
                    if (self.constraints.matchChildren) {
                        for (var i=0,j=self.constraints.matchChildren.length; i<j; i++) {
                            ...
                            if (subschema.pattern.test (name))
                                return subschema.validate (childValue, postValidate);
                        }
                    }

                    if (self.constraints.extras)
                        return self.constraints.extras.validate (childValue, postValidate);

                            if (self.constraints.keyFormat)
                                try {
                                    validateFormat (self.constraints.keyFormat, name);
                                    return postValidate();
                                } catch (err) {
                                    return callback (new ValidationError (
                                        ...
                                        name,
                                        self.path,
                                        'found improperly formatted key'
                                    ));
                                }
    ...
    number:     function (value, callback) {
        ...
        if (this.constraints.multiple && value % this.constraints.multiple)
            throw new ValidationError (
                'INVALID',
                this.constraints.multiple,
                value,
                this.path,
                'Number was not an appropriate multiple'
            );
