...
var RE_HOSTNAME = new RegExp ('^[^\\.]{0,63}(?:\\.[^\\.]{0,63})*$');

// RFC 1129 internet fqdn
var RE_FQDN = /^[a-zA-Z\d][a-zA-Z\d-]*(?:\.[a-zA-Z\d-]*[a-zA-Z\d])+$/;
...
module.exports.validate = function (type, str) {
    ...
    if (type == 'hostname') {
        console.log ('hostname', str, str.match (RE_HOSTNAME), Buffer.byteLength (str));
        if (Buffer.byteLength (str) > 255 || !str.match (RE_HOSTNAME))
            throw new ValidationError (
                'FORMAT'
            );
        return;
